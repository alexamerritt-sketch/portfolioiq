...
const SECTORS = ["Technology", "Healthcare", "Financials", "Consumer", "Energy", "Utilities", "Materials", "Industrials", "Real Estate", "Communication", "ETF - Broad Market", "ETF - Technology", "ETF - Financials", "ETF - Healthcare", "ETF - Energy", "ETF - Bonds", "ETF - International", "ETF - Commodities", "Crypto"];
...
const initialHoldings = [
  ...
  { id: 4, ticker: "SPY", name: "S&P 500 ETF", value: 8000, sector: "ETF - Broad Market", assetType: "ETF", dividendYield: 1.3, expenseRatio: 0.09 },
...
function getSectorColor(sector) {
  ...
  if (sector.startsWith("ETF")) return "#1d4ed8";
  return "#166534";
}
...
function calcMetrics(holdings) {
  ...
  // Group by broad category for analysis
  const sectorMap = {};
  holdings.forEach(h => {
    const broadSector = h.sector.startsWith("ETF") ? "ETF" : h.sector;
    sectorMap[broadSector] = (sectorMap[broadSector] || 0) + h.value;
  });
  const detailedSectorMap = {};
  holdings.forEach(h => { detailedSectorMap[h.sector] = (detailedSectorMap[h.sector] || 0) + h.value; });

  const sectorPcts = Object.entries(detailedSectorMap).map(([s, v]) => ({ sector: s, pct: (v / total) * 100 })).sort((a, b) => b.pct - a.pct);
  ...
  const divScore = Math.round(
    Math.min(numSectors / 7, 1) * 35 +
    Math.max(0, (1 - (topSectorPct - 20) / 60)) * 35 +
    Math.min(holdings.length / 10, 1) * 20 +
    (cryptoPct < 30 ? 10 : cryptoPct < 50 ? 5 : 0)
  );
  ...
  const riskScore = sectorPcts.reduce((s, sp) => {
    const broad = sp.sector.startsWith("ETF") ? "ETF" : sp.sector;
    return s + ((riskW[broad] || 5) * (sp.pct / 100));
  }, 0);
  ...
  const riskPct = Math.min((riskScore / 10) * 100, 95);
  ...
  if (cryptoPct > 40) flags.push({ type: "warning", msg: `Crypto is ${cryptoPct.toFixed(0)}% of your portfolio — high volatility risk` });
  ...
  const rebalance = [];
  const missing = ["Technology","Healthcare","Financials","Consumer","Energy"].filter(s => !detailedSectorMap[s] && !Object.keys(detailedSectorMap).some(k => k.startsWith("ETF")));
  ...
  if (cryptoPct > 30) rebalance.push({ type: "reduce", msg: `Consider reducing crypto to under 20% of your portfolio` });
  ...
  return { total, sectorPcts, divScore, scoreLabel, scoreColor, wDividend, wExpense, annualDividend, monthlyDividend, annualFee, riskScore, riskLabel, riskColor, riskPct, grade, gradeColor, numSectors, flags, rebalance, topSector, topSectorPct, cryptoPct, etfPct, stockPct };
}
      ...
      <div style={{ marginBottom: 28 }}>
        {steps.map((s, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14, opacity: step > i ? 1 : 0.3, transition: "opacity 0.4s", justifyContent: "center" }}>
            <div style={{ width: 24, height: 24, borderRadius: "50%", background: step > i ? "#166534" : "#d4c9a8", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: "#f5f0e4", fontWeight: 700, transition: "background 0.4s", flexShrink: 0 }}>
              {step > i ? "✓" : i + 1}
            </div>
...
function LicenseModal({ onUnlock, onClose }) {
  ...
  async function handleCheck() {
    ...
    if (result.valid) {
      ...
      setError("Invalid license key. Please check your Whop purchase email.");
    }
    setChecking(false);
  }
  return (
...
function AddHoldingForm({ onAdd, onCancel }) {
  ...
  async function handleTickerBlur() {
    ...
    setLooking(true);
    setLookupStatus("Looking up ticker...");
    ...
    if (data.found) {
      ...
      setLookupStatus("✓ Data auto-filled from live market");
    } else {
      setLookupStatus("Ticker not found — fill in manually");
    }
    setLooking(false);
  }
  ...
  return (
    <div style={{ background: "#f0ebe0", border: "1px solid #c4b896", borderRadius: 14, padding: 20, marginBottom: 16 }}>
      <div style={{ fontSize: 12, fontWeight: 700, color: "#6b5f3e", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 14 }}>Add Holding</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "120px 1fr 140px", gap: 10, marginBottom: 10 }}>
        <div>
          ...
          <input placeholder="AAPL" value={ticker} onChange={e => setTicker(e.target.value.toUpperCase())}
            ...
            style={{ fontFamily: "'DM Mono', monospace", fontWeight: 600, textTransform: "uppercase" }} />
        </div>
        <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 100px 120px 120px", gap: 10, marginBottom: 14 }}>
        <div>
        ...
        <div>
          ...
          <input placeholder="0.00" type="number" value={dividendYield} onChange={e => setDividendYield(e.target.value)}
            disabled={assetType === "Crypto"} style={{ opacity: assetType === "Crypto" ? 0.4 : 1 }} />
        </div>
        <div>
          <div style={{ fontSize: 11, color: "#8a7d5a", marginBottom: 4 }}>Exp. Ratio % <span style={{ color: "#b4a882" }}>(auto)</span></div>
          <input placeholder="0.00" type="number" value={expenseRatio} onChange={e => setExpenseRatio(e.target.value)}
            disabled={assetType === "Crypto" || assetType === "Stock"} style={{ opacity: (assetType === "Crypto" || assetType === "Stock") ? 0.4 : 1 }} />
        </div>
      </div>

      <div style={{ display: "flex", gap: 10 }}>
        <button onClick={handleAdd} disabled={!ticker || !value}
          style={{ background: ticker && value ? "#166534" : "#d4c9a8", color: ticker && value ? "#f5f0e4" : "#8a7d5a", border: "none", padding: "10px 24px", borderRadius: 8, fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 13, cursor: ticker && value ? "pointer" : "not-allowed", transition: "all 0.2s" }}>
          Add to Portfolio
        </button>
        <button onClick={onCancel} style={{ background: "transparent", color: "#8a7d5a", border: "1px solid #c4b896", padding: "10px 16px", borderRadius: 8, fontFamily: "'DM Sans', sans-serif", fontSize: 13, cursor: "pointer" }}>
          Cancel
        </button>
      </div>
...
export default function App() {
  ...
  function handleAdd(holding) { setHoldings(prev => [...prev, holding]); setShowAdd(false); setStage("input"); }
  ...
  function handleUnlock() { setUnlocked(true); setShowModal(false); }
  const total = holdings.reduce((s, h) => s + h.value, 0);
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .hr { transition: background 0.15s; }
        .hr:hover { background: #ede8d8 !important; }
        ...
        .flag-w { background: #fffbeb; border: 1px solid #fcd34d; color: #92400e; border-radius: 8px; padding: 10px 14px; font-size: 13px; display: flex; align-items: flex-start; gap: 8px; line-height: 1.5; }
        .flag-o { background: #f0fdf4; border: 1px solid #86efac; color: #14532d; border-radius: 8px; padding: 10px 14px; font-size: 13px; display: flex; align-items: flex-start; gap: 8px; line-height: 1.5; }
        .rb-r { background: #fff1f2; border: 1px solid #fecaca; color: #991b1b; border-radius: 8px; padding: 10px 14px; font-size: 13px; display: flex; gap: 10px; line-height: 1.5; }
        .rb-a { background: #f0fdf4; border: 1px solid #bbf7d0; color: #14532d; border-radius: 8px; padding: 10px 14px; font-size: 13px; display: flex; gap: 10px; line-height: 1.5; }
        ...
        .slabel { font-size: 10px; color: #8a7d5a; text-transform: uppercase; letter-spacing: 0.12em; margin-bottom: 12px; font-weight: 700; }
        @media (max-width: 640px) {
          .g4 { grid-template-columns: 1fr 1fr; }
          .g3 { grid-template-columns: 1fr 1fr; }
          .g2 { grid-template-columns: 1fr; }
          .sf { flex-direction: column !important; }
      ...
      {!unlocked ? (
        <div style={{ background: "#1a2e1a", padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "center", gap: 12, flexWrap: "wrap" }}>
          <span style={{ fontSize: 13, color: "#d4c9a8" }}>📊 Free preview — full analysis locked</span>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "center" }}>
            <a href={WHOP_URL} target="_blank" rel="noopener noreferrer" style={{ background: "#15803d", color: "#f5f0e4", padding: "6px 16px", borderRadius: 6, fontSize: 12, fontWeight: 700, textDecoration: "none" }}>Unlock {PRICE} →</a>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 20px 0" }}>
              ...
              <div className="ht" style={{ background: "#faf7f0", border: "1px solid #d4c9a8", borderRadius: 12, overflow: "hidden" }}>
                ...
                {holdings.map((h, i) => (
                  <div key={h.id} className="hr" style={{ display: "grid", gridTemplateColumns: "70px 1fr 80px 110px 80px 70px 70px 36px", padding: "12px 16px", borderBottom: i < holdings.length - 1 ? "1px solid #ede8d8" : "none", alignItems: "center" }}>
                    ...
                    <span>
                      <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 4, fontWeight: 600, background: h.assetType === "Crypto" ? "#fef3c7" : h.assetType === "ETF" ? "#dbeafe" : "#f0fdf4", color: h.assetType === "Crypto" ? "#92400e" : h.assetType === "ETF" ? "#1e40af" : "#14532d" }}>
                        {h.assetType}
                      </span>
                    </span>
          ...
          {holdings.length > 0 && (
            <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
              {[
                { label: "Stocks", count: holdings.filter(h => h.assetType === "Stock").length, color: "#f0fdf4", text: "#14532d" },
                { label: "ETFs", count: holdings.filter(h => h.assetType === "ETF").length, color: "#dbeafe", text: "#1e40af" },
                { label: "Crypto", count: holdings.filter(h => h.assetType === "Crypto").length, color: "#fef3c7", text: "#92400e" },
              ].filter(p => p.count > 0).map(p => (
                <div key={p.label} style={{ background: p.color, color: p.text, padding: "5px 12px", borderRadius: 20, fontSize: 12, fontWeight: 600 }}>
                  {p.count} {p.label}
                </div>
              ))}
            </div>
          ...
          {stage === "results" && metrics && (
            <div ref={resultsRef} style={{ opacity: animIn ? 1 : 0, transition: "opacity 0.5s ease" }}>
              ...
              {/* TOP ROW — 4 big stat cards (Concept A) */}
              <div className="g4">
              ...
              <div className="g3">
                ...
                <StatCard label="Sectors Covered" value={`${metrics.numSectors}`} sub={`Out of ${SECTORS.length} total categories`} color={metrics.numSectors >= 5 ? "#15803d" : metrics.numSectors >= 3 ? "#d97706" : "#dc2626"} />
              </div>
              ...
              <div style={{ position: "relative" }}>
                  ...
                  <div style={{ position: "absolute", inset: 0, zIndex: 10, background: "rgba(245,240,228,0.8)", backdropFilter: "blur(8px)", borderRadius: 16, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, minHeight: 400 }}>
                    <div style={{ background: "#faf7f0", border: "1px solid #d4c9a8", borderRadius: 16, padding: "36px 28px", textAlign: "center", maxWidth: 400, width: "100%", boxShadow: "0 16px 40px rgba(26,46,26,0.12)" }}>
                      ...
                      <div style={{ textAlign: "left", marginBottom: 24, display: "flex", flexDirection: "column", gap: 7 }}>
                        {["Risk flags & specific warnings", "Full sector breakdown vs S&P 500", "Detailed rebalancing action plan", "Crypto & ETF specific analysis", "Fee optimization suggestions"].map((f, i) => (
                          <div key={i} style={{ fontSize: 13, color: "#3d3520", display: "flex", gap: 10 }}>
                            <span style={{ color: "#15803d", fontWeight: 700 }}>✓</span> {f}
                          </div>
                        ))}
                      </div>
                      <a href={WHOP_URL} target="_blank" rel="noopener noreferrer" style={{ display: "block", background: "#166534", color: "#f5f0e4", padding: "15px", borderRadius: 9, fontSize: 15, fontWeight: 700, textDecoration: "none", marginBottom: 10 }}>Unlock Full Access — {PRICE} →</a>
                      <button onClick={() => setShowModal(true)} style={{ background: "transparent", color: "#8a7d5a", border: "1px solid #c4b896", padding: "11px", borderRadius: 9, fontSize: 13, cursor: "pointer", width: "100%", fontFamily: "'DM Sans', sans-serif" }}>I already purchased — enter key</button>
                    </div>
                  </div>
                )}
                ...
                <div style={{ filter: unlocked ? "none" : "blur(6px)", pointerEvents: unlocked ? "auto" : "none", userSelect: unlocked ? "auto" : "none" }}>
                  ...
                  <div style={{ marginBottom: 18 }}>
                    <div className="slabel">Risk Flags & Insights</div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {metrics.flags.map((f, i) => (
                        <div key={i} className={f.type === "warning" ? "flag-w" : "flag-o"}>
                  ...
                  <div className="card" style={{ marginBottom: 18 }}>
                    <div className="slabel">Sector & Asset Breakdown vs S&P 500</div>
                    ...
                    <div className="card" style={{ marginBottom: 18, border: "1px solid #86efac", background: "#f0fdf4" }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                        {metrics.rebalance.map((r, i) => (
                          <div key={i} className={r.type === "reduce" ? "rb-r" : "rb-a"}>
