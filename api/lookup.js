const TICKER_DB = {
  ...
  "HYG":  { name: "iShares High Yield Corp Bond ETF", sector: "ETF - Bonds", assetType: "ETF", dividendYield: 5.80, expenseRatio: 0.48 },
...
export default async function handler(req, res) {
  ...
  if (TICKER_DB[symbol]) {
    return res.status(200).json({ ...TICKER_DB[symbol], found: true });
  }
  ...
  try {
    ...
    if (!profile || !profile.name) return res.status(200).json({ found: false });
    const ind = (profile.finnhubIndustry || "").toLowerCase();
    const n = (profile.name || "").toLowerCase();
    let sector = "Technology";
    if (n.includes("etf") || n.includes("fund")) sector = "ETF - Broad Market";
    else if (ind.includes("tech") || ind.includes("software") || ind.includes("semi")) sector = "Technology";
    else if (ind.includes("health") || ind.includes("pharma") || ind.includes("bio")) sector = "Healthcare";
    else if (ind.includes("bank") || ind.includes("financ") || ind.includes("insur")) sector = "Financials";
    else if (ind.includes("retail") || ind.includes("consumer") || ind.includes("food")) sector = "Consumer";
    else if (ind.includes("energy") || ind.includes("oil") || ind.includes("gas")) sector = "Energy";
    else if (ind.includes("util")) sector = "Utilities";
    else if (ind.includes("material") || ind.includes("mining")) sector = "Materials";
    else if (ind.includes("industrial") || ind.includes("aerospace")) sector = "Industrials";
    else if (ind.includes("real estate") || ind.includes("reit")) sector = "Real Estate";
    else if (ind.includes("media") || ind.includes("telecom")) sector = "Communication";
    return res.status(200).json({
      name: profile.name,
      sector,
      assetType: n.includes("etf") || n.includes("fund") ? "ETF" : "Stock",
      dividendYield: parseFloat((metrics?.metric?.dividendYieldIndicatedAnnual || 0).toFixed(2)),
      expenseRatio: 0,
      found: true
    });
